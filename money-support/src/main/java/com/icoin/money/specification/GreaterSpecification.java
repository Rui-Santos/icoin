 ...
 */
public class GreaterSpecification extends CompositeSpecification<BigMoney> {
    private final BigMoney threshold;

    public GreaterSpecification(BigMoney threshold) {
        notNull(threshold);
    ...
    public boolean isSatisfiedBy(BigMoney amount) {
        notNull(amount);
        return threshold.isGreaterThan(amount);
    }
}

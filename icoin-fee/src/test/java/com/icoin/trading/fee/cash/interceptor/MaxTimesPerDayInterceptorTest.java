 ...
 */
public class MaxTimesPerDayInterceptorTest {
    @Test
    public void testAllowed() throws Exception {
        ...
        when(cashRepository.findByUserId(anyString(), any(Date.class))).
                thenReturn(ImmutableList.of(cash1), ImmutableList.of(cash1, cash2));

        DueDateService dateService = mock(DueDateService.class);

        final MaxTimesPerDayInterceptor interceptor
                = new MaxTimesPerDayInterceptor();

        interceptor.setCashRepository(cashRepository);
        interceptor.setService(dateService);
        interceptor.setMaxTime(2);
    ...
    public void testDisallowed() throws Exception {
        ...
        assertThat(validationCode, is(ValidationCode.EXCEEDED_MAX_TIMES_PER_DAY));
